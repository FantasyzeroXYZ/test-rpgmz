/* @ts-self-types="./rm_wasm_vfs.d.ts" */
 ...
 * @returns {boolean}
 */
export function has_file(path) {
    const ptr0 = passStringToWasm0(path, wasm.__wbindgen_export, wasm.__wbindgen_export2);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.has_file(ptr0, len0);
    return ret !== 0;
}
...
export function write_file(path, data) {
    ...
    const len1 = WASM_VECTOR_LEN;
    wasm.write_file(ptr0, len0, ptr1, len1);
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('rm_wasm_vfs_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
